...
class CChan:
    ...
    def GetStockAPI(self):
        ...
        # 安全化: 原实现按字符串动态 import 任意模块, 已改为白名单(vendor patch)
        package_info = self.data_src.split(":")[1]
        package_name, cls_name = package_info.split(".")
        _ALLOWED = {"MEM_API", "csvAPI"}
        if package_name not in _ALLOWED:
            raise CChanException("custom data src not in allowlist", ErrCode.SRC_DATA_TYPE_ERR)
        import sys as _sys
        module = _sys.modules.get(f"DataAPI.{package_name}")
        if module is None:
            from DataAPI import csvAPI as module
    ...
    # 安全化: 已移除 pickle 序列化辅助(chan_dump_pickle/chan_load_pickle, 本技能不使用; vendor patch)
