...
class CChanConfig:
    ...
    def set_bsp_config(self, conf):
        ...
        # 安全化: 原实现用 exec 拼接字符串装载配置, 已改为直接方法调用(vendor patch, 行为等价)
        for k, v in conf.items():
            if k.endswith("-buy"):
                self.bs_point_conf.b_conf.set(k.replace("-buy", ""), v)
            elif k.endswith("-sell"):
                self.bs_point_conf.s_conf.set(k.replace("-sell", ""), v)
            elif k.endswith("-segbuy"):
                self.seg_bs_point_conf.b_conf.set(k.replace("-segbuy", ""), v)
            elif k.endswith("-segsell"):
                self.seg_bs_point_conf.s_conf.set(k.replace("-segsell", ""), v)
            elif k.endswith("-seg"):
                self.seg_bs_point_conf.b_conf.set(k.replace("-seg", ""), v)
                self.seg_bs_point_conf.s_conf.set(k.replace("-seg", ""), v)
            elif k in args:
